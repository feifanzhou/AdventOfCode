defmodule AOC.D04 do
  ...
  def part_one do
    Enum.count(parsed_input(), fn passport_map ->
      Enum.all?(@required_keys, fn key -> Map.has_key?(passport_map, key) end)
    end)
  end

  def part_two do
    Enum.count(parsed_input(), &valid_passport?/1)
  end

  defp valid_passport?(passport_map) do
    if Enum.all?(@required_keys, fn key -> Map.has_key?(passport_map, key) end) do
      valid_byr?(passport_map) &&
      valid_iyr?(passport_map) &&
      valid_eyr?(passport_map) &&
      valid_hgt?(passport_map) &&
      valid_hcl?(passport_map) &&
      valid_ecl?(passport_map) &&
      valid_pid?(passport_map)
    else
      false
    end
  end
  ...
  defp valid_byr?(%{"byr" => byr}) do
    String.match?(byr, ~r/^[[:digit:]]{4}$/) and Enum.member?(1920..2002, String.to_integer(byr))
  end

  defp valid_iyr?(%{"iyr" => iyr}) do
    String.match?(iyr, ~r/^[[:digit:]]{4}$/) and Enum.member?(2010..2020, String.to_integer(iyr))
  end

  defp valid_eyr?(%{"eyr" => eyr}) do
    String.match?(eyr, ~r/^[[:digit:]]{4}$/) and Enum.member?(2020..2030, String.to_integer(eyr))
  end

  @cm_regex ~r/^(?<cm>[[:digit:]]{3})cm$/
  @in_regex ~r/^(?<in>[[:digit:]]{2})in$/
  defp valid_hgt?(%{"hgt" => hgt}) do
    capture_result = Regex.named_captures(@cm_regex, hgt) || Regex.named_captures(@in_regex, hgt)
    case capture_result do
      %{"cm" => cm_digits} -> Enum.member?(150..193, String.to_integer(cm_digits))
      %{"in" => in_digits} -> Enum.member?(59..76, String.to_integer(in_digits))
      _else -> false
    end
  end

  defp valid_hcl?(%{"hcl" => hcl}) do
    String.match?(hcl, ~r/^#[[:xdigit:]]{6}$/)
  end

  @valid_eye_colors MapSet.new(~w[amb blu brn gry grn hzl oth])
  defp valid_ecl?(%{"ecl" => ecl}) do
    Enum.member?(@valid_eye_colors, ecl)
  end

  defp valid_pid?(%{"pid" => pid}) do
    String.match?(pid, ~r/^[[:digit:]]{9}$/)
  end
  ...
  def parsed_input do
    ...
    |> String.trim()
    |> String.split("\n\n")
    |> Enum.map(fn passport_entry ->
      String.split(passport_entry, ~r/[[:space:]]/)
      |> Enum.map(fn kv_pair_string ->
        String.split(kv_pair_string, ":")
      end)
