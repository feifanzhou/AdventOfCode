defmodule AOC.D07 do
  def part_one do
    split_input()
    |> Enum.map(&parse_input_line/1)
    |> invert_contain_graph()
    |> IO.inspect()
    |> find_paths("shiny gold")
    |> IO.inspect()
    |> MapSet.size()
  end

  defp find_paths(contain_graph, target) do
    direct_children = Map.get(contain_graph, target, MapSet.new())

    descendents =
      Enum.map(direct_children, fn child_target -> find_paths(contain_graph, child_target) end)
      |> Enum.reduce(MapSet.new(), fn children, acc -> MapSet.union(children, acc) end)

    MapSet.union(direct_children, descendents)
  end

  defp invert_contain_graph(contains_list) do
    Enum.reduce(contains_list, %{}, fn {outer_type, inners}, graph ->
      Enum.reduce(inners, graph, fn {_num, inner_type}, graph ->
        new_inner_value =
          if Map.has_key?(graph, inner_type) do
            Map.get(graph, inner_type) |> MapSet.put(outer_type)
          else
            MapSet.new() |> MapSet.put(outer_type)
          end

        Map.put(graph, inner_type, new_inner_value)
      end)
    end)
  end
  ...
  def part_two do
    split_input()
    |> Enum.map(&parse_input_line/1)
    |> regular_contain_graph()
    |> IO.inspect()
    |> number_of_sub_bags("shiny gold")
  end

  defp number_of_sub_bags(contains_list, target) do
    inners = Map.get(contains_list, target, [])

    if length(inners) == 0 do
      0
    else
      inners
      |> Enum.map(fn {count, sub_target} ->
        # +1 for itself
        (number_of_sub_bags(contains_list, sub_target) + 1) * count
      end)
      |> Enum.reduce(&+/2)
    end
  end

  defp regular_contain_graph(contains_list) do
    Enum.into(contains_list, %{})
  end

  defp parse_input_line(line) do
    [outer, inners] = String.split(line, " contain ")
    [outer, _] = String.split(outer, " bags")
    inners = if inners == "no other bags.", do: [], else: String.split(inners, ", ")

    inners =
      Enum.map(inners, fn inner ->
        [num, adj, color, _] = String.split(inner)
        {String.to_integer(num), "#{adj} #{color}"}
      end)
