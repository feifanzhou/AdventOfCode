defmodule AOC.D11 do
  def part_one do
    split_input() |> apply_rules_until_stable() |> count_occupied_seats()
  end

  defp count_occupied_seats(grid) do
    Enum.reduce(0..(length(grid) - 1), 0, fn y, sum ->
      row = Enum.at(grid, y)

      sum +
        Enum.reduce(0..(length(row) - 1), 0, fn x, subsum ->
          if char_at(grid, {x, y}) == "#", do: subsum + 1, else: subsum
        end)
    end)
  end

  defp apply_rules_until_stable(input_grid) do
    new_grid = apply_rules(input_grid)
    if new_grid == input_grid, do: new_grid, else: apply_rules_until_stable(new_grid)
  end

  defp apply_rules(grid) do
    0..(length(grid) - 1)
    |> Enum.reduce([], fn y, new_grid ->
      row = Enum.at(grid, y)

      new_row =
        0..(length(row) - 1)
        |> Enum.reduce([], fn x, new_row ->
          current_pos = {x, y}
          current_char = char_at(grid, current_pos)

          new_char =
            cond do
              current_char == "L" && no_occupied_adjacent_seats?(grid, current_pos) ->
                "#"

              current_char == "#" && four_or_more_occupied_adjacent_seats?(grid, current_pos) ->
                "L"
  ...
  defp no_occupied_adjacent_seats?(grid, {x, y} = pos) do
    valid_adjacencies(grid, pos)
    |> Enum.all?(fn {dx, dy} ->
      adj_pos = {x + dx, y + dy}
      char_at(grid, adj_pos) == "L" || char_at(grid, adj_pos) == "."
    end)
  end

  defp valid_adjacencies(grid, {x, y}) do
    grid_max_y = length(grid) - 1
    grid_max_x = length(Enum.at(grid, 0)) - 1

    @adjacent_deltas
    |> Enum.filter(fn {dx, dy} ->
      new_x = x + dx
      new_y = y + dy
      new_x >= 0 && new_x <= grid_max_x && new_y >= 0 && new_y <= grid_max_y
    end)
  end

  defp four_or_more_occupied_adjacent_seats?(grid, {x, y} = pos) do
    valid_adjacencies(grid, pos)
    |> Enum.filter(fn {dx, dy} ->
      adj_pos = {x + dx, y + dy}
      char_at(grid, adj_pos) == "#"
    end)
    |> length() >= 4
  end
